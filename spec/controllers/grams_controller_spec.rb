...
RSpec.describe GramsController, type: :controller do
  describe "grams#destroy action" do
    ...
    it "should return a 404 message if we connot find a gram with the id that is specified" do
      user = FactoryGirl.create(:user)
      sign_in user
      delete :destroy, id: 'SPaceDOg'
      expect(response).to have_http_status(:not_found)
    end
  end

  describe "grams#update action" do
    it "should not let users who didn not create the gram update it" do
      gram = FactoryGirl.create(:gram)
      user = FactoryGirl.create(:user)
      sign_in user
      patch :update, id: gram.id, gram: { message: 'bling'}
      expect(response).to have_http_status(:forbidden)
    end

    it "shouldn't let unauthenticated users create a gram" do
      gram = FactoryGirl.create(:gram)
      patch :update, id: gram.id, gram: { message: "hihowareya" }
      expect(response).to redirect_to new_user_session_path
    end
    ...
    it "should have http 404 error if the gram cannot be found" do
      ...
      patch :update, id: "YOLOCOUNTY", gram: {message: 'Changed'}
      expect(response).to have_http_status(:not_found)
    end
